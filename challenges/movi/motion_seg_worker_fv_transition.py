...
parser.add_argument("--camera", choices=["fixed_random", "linear_movement", "heavy_movement"],
...
with open("./built_datasets/motion_seg/train_background.txt", 'w') as f:
  for train_background in train_backgrounds:
    f.write(train_background+'\n')

with open("./built_datasets/motion_seg/test_background.txt", 'w') as f:
  for test_background in test_backgrounds:
    f.write(test_background+'\n')
'''
with open("./built_datasets/motion_seg/train_background.txt") as f:
  train_backgrounds = [l.strip() for l in f.readlines()]

with open("./built_datasets/motion_seg/test_background.txt") as f:
  test_backgrounds = [l.strip() for l in f.readlines()]
...
if FLAGS.camera == "fixed_random":
  ...
  scene.camera.position = tuple(xyz.tolist())
  scene.camera.look_at((0, 0, xyz[2]))
...
elif FLAGS.camera == "heavy_movement":
  camera_start, camera_end = get_linear_camera_motion_start_end(
      movement_speed=FLAGS.max_camera_movement
  )
  # linearly interpolate the camera position between these two points
  # while keeping it focused on the center of the scene
  # we start one frame early and end one frame late to ensure that
      ...
      if frame % 4 == 1:
        ...
        if i % 2 == 1:
          ...
          scene.camera.position = (interp * np.array(camera_start) +
                                  (1 - interp) * np.array(camera_end))
        scene.camera.look_at((0, 0, 0))
        scene.camera.keyframe_insert("position", real_frame)
        scene.camera.keyframe_insert("quaternion", real_frame)
        real_frame += 1
  print (real_frame)
...
with open("./built_datasets/motion_seg/train_gso.txt", 'w') as f:
  for train_obj in train_split:
    f.write(train_obj+'\n')

with open("./built_datasets/motion_seg/test_gso.txt", 'w') as f:
  for test_obj in test_split:
    f.write(test_obj+'\n')
'''
with open("./built_datasets/motion_seg/train_gso.txt") as f:
  train_split = [l.strip() for l in f.readlines()]

with open("./built_datasets/motion_seg/test_gso.txt") as f:
  test_split = [l.strip() for l in f.readlines()]
